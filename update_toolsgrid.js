...
const targetPath = path.join(__dirname, 'src', 'components', 'ToolsGrid.tsx');
let content = fs.readFileSync(targetPath, 'utf8');

// 1. Update imports
const newImports = \`HeartPulse, Share2, Smartphone, Dices, FileSpreadsheet, Presentation, Flame, PieChart, Wine, Tags, Youtube, Minimize2, MonitorSmartphone, Info, Battery\`;
content = content.replace(/Calculator as CalcIcon, LineChart, MoveHorizontal/, \`Calculator as CalcIcon, LineChart, MoveHorizontal, \${newImports}\`);

// 2. Add Document Converters to Document & Text
const docTools = \`
...
content = content.replace(/subtitle: "Convert MD to raw HTML code.".*\\n.*\\].*\\},/s, match => {
  return match.replace(/\\n\\s*\\]/, \`\${docTools}\\n      ]\`);
});

// 3. Update Math, Finance, Data and add the new categories
const newCategories = \`
    {
      id: "math-finance",
      title: "Math & Finance",
      tools: [
        ...
        { title: "Rule of 72", subtitle: "Calculate years to double your money.", icon: <LineChart className="w-12 h-12" />, iconColor: "text-amber-500", rating: 5, isBeta: false },
    ...
    },
    {
      ...
      tools: [
        ...
        { title: "YT Thumbnail", subtitle: "Download YouTube thumbnails.", icon: <Youtube className="w-12 h-12" />, iconColor: "text-red-500", rating: 5, isBeta: true },
...
content = content.replace(/\\{\\s*id: "math-finance"[\\s\\S]*?\\}\\s*\\];/m, newCategories + "\\n  ];");

fs.writeFileSync(targetPath, content);
console.log('Done updating ToolsGrid.tsx');
